...
let rec generate_expr = function
  ...
  | Dot -> generate_dot ()
  | Character a -> generate_char a
  | Or lst -> generate_or lst
  | And lst -> generate_and lst
  | Multiple (expr, quant) -> generate_multiple expr quant
...
and generate_char a = String.make 1 a
...
and generate_bounded_range expr start_quant end_quant =
  let _end_quant = if start_quant = 0 then end_quant + 1 else end_quant in
